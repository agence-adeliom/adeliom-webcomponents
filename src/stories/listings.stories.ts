...
const card = html`
  <awc-card class="col-span-full md:col-span-6 lg:col-span-4 basic-card" shadow bordered>
    ...
    <div class="flex flex-col gap-6">
      <span class="text-xl font-bold"> Titre de la card </span>
...
const reviewCard = html`
  <!-- review - start -->
  <awc-card
    class="col-span-2 lg:col-span-1 border border-[var(--awc-card-border-color)] py-6 px-6 rounded"
    style="--padding: 0"
  >
    ...
    <div slot="footer" class="mt-4 flex items-center gap-3">
      ...
      <div class="flex flex-col">
        ...
        <span class="block text-sm text-paragraph"> Lorem ipsum </span>
      </div>
    </div>

    <h4 class="text-title font-bold mb-1">Titre de la card</h4>

    <p class="text-paragraph">Accumsan egestas molestie dignissim bibendum leo nunc. Enim in amet amet amet orci.</p>
  </awc-card>
  <!-- review - end -->
`;

const iconSelect = html`
  <awc-select multiple placeholder="Ex : Exemple" clearable>
...
const basicSelect = html`
  <awc-select placeholder="Ex : Exemple" multiple clearable>
    <awc-option value="option-1">Option 1</awc-option>
    <awc-option value="option-2">Option 2</awc-option>
    <awc-option value="option-3">Option 3</awc-option>
    <awc-option value="option-4">Option 4</awc-option>
  ...
  render: () =>
    html` <div class="container mx-auto px-4 md:px-8">
        ...
        <awc-drawer placement="start" class="filters_1-drawer">
          ...
          <div class="lg:hidden flex flex-col py-4 gap-4">${iconSelect} ${basicSelect} ${basicSelect}</div>
        </awc-drawer>
        <!-- End other filters tab -->

        <!-- Filters details start -->
        <div class="w-full flex items-center justify-between mt-6 lg:mt-12">
          <p class="text-paragraph"><strong>XX</strong> résultats</p>
          <awc-dropdown>
  ...
  render: () => html`
    <div class="container mx-auto px-4 md:px-8">
      ...
      <div class="grid grid-cols-2 gap-6 mt-4 lg:mt-6 relative">
        ${reviewCard} ${reviewCard} ${reviewCard} ${reviewCard} ${reviewCard} ${reviewCard} ${reviewCard} ${reviewCard}
...
export const Listing3: StoryObj = {
  name: 'Listing 3',
  render: () => html`
    <div class="container mx-auto px-4 md:px-8">${heading}</div>
