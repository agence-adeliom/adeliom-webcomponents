...
import type AWCSplitPanel from './split-panel.js';

const DIVIDER_WIDTH_IN_PX = 4;

const getPanel = (splitPanel: AWCSplitPanel, testid: string): HTMLElement => {
  const startPanel = queryByTestId<HTMLElement>(splitPanel, testid);
  expect(startPanel).not.to.be.null;
  return startPanel!;
};

const getPanelWidth = (splitPanel: AWCSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { width } = panel.getBoundingClientRect();
  return width;
};

const getPanelHeight = (splitPanel: AWCSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { height } = panel.getBoundingClientRect();
  return height;
};

const getDivider = (splitPanel: AWCSplitPanel): Element => {
  const divider = splitPanel.shadowRoot?.querySelector('[part="divider"]');
  expect(divider).not.to.be.null;
  return divider!;
};

describe('<awc-split-panel>', () => {
  afterEach(async () => {
    await resetMouse();
  });

  it('should render a component', async () => {
    const splitPanel = await fixture(html` <awc-split-panel></awc-split-panel> `);
  ...
  describe('panel sizing horizontal', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<AWCSplitPanel>(
        html`<awc-split-panel>
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<AWCSplitPanel>(
        html`<awc-split-panel position="25">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </awc-split-panel>`
      );
    ...
    it('emits the awc-reposition	event on position change', async () => {
      const splitPanel = await fixture<AWCSplitPanel>(
        html`<awc-split-panel>
          <div slot="start">Start</div>
          <div slot="end">End</div>
      ...
      const repositionPromise = oneEvent(splitPanel, 'awc-reposition');
      splitPanel.position = 10;
  ...
  describe('panel sizing vertical', () => {
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<AWCSplitPanel>(
        html`<awc-split-panel position="25" vertical style="height: 400px;">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </awc-split-panel>`
      );
    ...
    it('cannot be resized if disabled', async () => {
      const splitPanel = await fixture<AWCSplitPanel>(
        html`<awc-split-panel disabled vertical style="height: 400px;">
          <div slot="start">Start</div>
          <div slot="end">End</div>
        </awc-split-panel>`
      );
