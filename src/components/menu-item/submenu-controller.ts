...
export class SubmenuController implements ReactiveController {
  private host: ReactiveControllerHost & AWCMenuItem;
  private popupRef: Ref<AWCPopup> = createRef();
  ...
  constructor(
    host: ReactiveControllerHost & AWCMenuItem,
  ...
  private addListeners() {
    ...
    if (!this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.addEventListener('awc-reposition', this.handlePopupReposition);
  ...
  private removeListeners() {
    ...
    if (this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.removeEventListener('awc-reposition', this.handlePopupReposition);
  ...
  private handleSubmenuEntry(event: KeyboardEvent) {
    ...
    for (const elt of submenuSlot.assignedElements()) {
      menuItems = elt.querySelectorAll("awc-menu-item, [role^='menuitem']");
  ...
  private handleClick = (event: MouseEvent) => {
    ...
    if (event.target === this.host) {
      ...
      (event.target.tagName === 'awc-menu-item' || event.target.role?.startsWith('menuitem'))
    ) {
      this.disableSubmenu();
    }
  };
  ...
  private handlePopupReposition = () => {
    const submenuSlot: HTMLSlotElement | null = this.host.renderRoot.querySelector("slot[name='submenu']");
    const menu = submenuSlot?.assignedElements({ flatten: true }).filter(el => el.localName === 'awc-menu')[0];
  ...
  renderSubmenu() {
    ...
    // Always render the slot, but conditionally render the outer <awc-popup>
    if (!this.isConnected) {
