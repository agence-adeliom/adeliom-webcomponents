...
export const Primary: Story = {
 ...
 export const SlideInFromStart: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "By default, drawers slide in from the end. To make the drawer slide in from the start, set the `placement` attribute to `start`."
      }
    }
  }
};
...
export const ContainedElement: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "By default, drawers slide out of their [containing block](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block#Identifying_the_containing_block), which is usually the viewport. To make a drawer slide out of a parent element, add the `contained` attribute to the drawer and apply `position: relative` to its parent.<br><br>Unlike normal drawers, contained drawers are not modal. This means they do not show an overlay, they do not trap focus, and they are not dismissible with [[Escape]]. This is intentional to allow users to interact with elements outside of the drawer."
      }
    }
  }
};
...
export const CustomSize: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Use the `--size` custom property to set the drawer's size. This will be applied to the drawer's width or height depending on its `placement`."
      }
    }
  }
};
...
export const PreventingDrawerFromClosing: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "By default, drawers will close when the user clicks the close button, clicks the overlay, or presses the [[Escape]] key. In most cases, the default behavior is the best behavior in terms of UX. However, there are situations where this may be undesirable, such as when data loss will occur.<br><br>To keep the drawer open in such cases, you can cancel the `awc-request-close` event. When canceled, the drawer will remain open and pulse briefly to draw the user's attention to it.<br><br>You can use `event.detail.source` to determine what triggered the request to close. This example prevents the drawer from closing when the overlay is clicked, but allows the close button or [[Escape]] to dismiss it."
      }
    }
  }
};
