 ...
 * @since 1.0
 */
export default class AWCMapIcon extends AWCLeafletElement {
  static isLeafletIcon(node: Element): node is AWCMapIcon {
    return node instanceof AWCMapIcon;
  }
  ...
  getIcon(): L.Icon {
    ...
    const shadowSize = shadowWidth && shadowHeight ? L.point(shadowWidth, shadowHeight) : undefined;

    const shadowAnchor = shadowAnchorX && shadowAnchorY ? L.point(shadowAnchorX, shadowAnchorY) : undefined;

    const popupAnchor = popupAnchorX && popupAnchorY ? L.point(popupAnchorX, popupAnchorY) : undefined;
