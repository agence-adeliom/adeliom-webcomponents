...
import SystemIcons from '@awc-storybook/static/assets/icons/icons.json';
...
Icons are lazy loaded by default. You can disable this behavior by setting the `lazy` attribute to `false`.

```html
<awc-icon name="icon-name-here" lazy="false"></awc-icon>
```
