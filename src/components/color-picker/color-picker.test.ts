...
describe('<awc-color-picker>', () => {
  describe('when the value changes', () => {
    it('should not emit awc-change or awc-input when the value is changed programmatically', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker></awc-color-picker> `);
      const color = 'rgb(255, 204, 0)';

      el.addEventListener('awc-change', () => expect.fail('awc-change should not be emitted'));
      el.addEventListener('awc-input', () => expect.fail('awc-change should not be emitted'));
      el.value = color;
      await el.updateComplete;
    });

    it('should emit awc-change and awc-input when the color grid selector is moved', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker></awc-color-picker> `);
    ...
    it('should emit awc-change and awc-input when the hue slider is moved', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit awc-change and awc-input when the opacity slider is moved', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker opacity></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit awc-change and awc-input when toggling the format', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker value="#fff"></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should render the correct swatches when passing a string of color values', async () => {
      const el = await fixture<AWCColorPicker>(html`
        <awc-color-picker swatches="red; #008000; rgb(0,0,255);"></awc-color-picker>
      `);
    ...
    it('should emit awc-change and awc-input when clicking on a swatch', async () => {
      const el = await fixture<AWCColorPicker>(html`
        <awc-color-picker swatches="red; green; blue;"></awc-color-picker>
      `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit awc-change and awc-input when selecting a color with the keyboard', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit awc-change and awc-input when selecting hue with the keyboard', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit awc-change and awc-input when selecting opacity with the keyboard', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker opacity></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit awc-change and awc-input when entering a value in the color input and pressing enter', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker opacity></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit awc-change and awc-input when entering a value in the color input and blurring the field', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker opacity></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
      const input = el.shadowRoot!.querySelector<HTMLElement>('[part~="input"]')!;
      const changeHandler = sinon.spy();
    ...
    it('should render the correct format when selecting a swatch of a different format', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker format="rgb"></awc-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
      const changeHandler = sinon.spy();
      const inputHandler = sinon.spy();
  ...
  it('should render in a dropdown', async () => {
    const el = await fixture<AWCColorPicker>(html` <awc-color-picker></awc-color-picker> `);
    const dropdown = el.shadowRoot!.querySelector('awc-dropdown');

    expect(dropdown).to.exist;
  });

  it('should not render in a dropdown when inline is enabled', async () => {
    const el = await fixture<AWCColorPicker>(html` <awc-color-picker inline></awc-color-picker> `);
    const dropdown = el.shadowRoot!.querySelector('awc-dropdown');
  ...
  it('should display a color when an initial value is provided', async () => {
    const el = await fixture<AWCColorPicker>(html` <awc-color-picker value="#000"></awc-color-picker> `);
    const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]');
  ...
  it('should display a color with opacity when an initial value with opacity is provided', async () => {
    const el = await fixture<AWCColorPicker>(html` <awc-color-picker opacity value="#ff000050"></awc-color-picker> `);
    const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
  ...
  it('should emit awc-focus when rendered as a dropdown and focused', async () => {
    const el = await fixture<AWCColorPicker>(html`
      <div>
    ...
    `);
    const colorPicker = el.querySelector('awc-color-picker')!;
    const trigger = colorPicker.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    const button = el.querySelector('button')!;
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();

    colorPicker.addEventListener('awc-focus', focusHandler);
    colorPicker.addEventListener('awc-blur', blurHandler);
  ...
  it('should emit awc-focus when rendered inline and focused', async () => {
    const el = await fixture<AWCColorPicker>(html`
      <div>
        <awc-color-picker inline></awc-color-picker>
        <button type="button">Click me</button>
      </div>
    `);
    const colorPicker = el.querySelector('awc-color-picker')!;
  ...
  it('should focus and blur when calling focus() and blur() and rendered as a dropdown', async () => {
    const colorPicker = await fixture<AWCColorPicker>(html` <awc-color-picker></awc-color-picker> `);
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();
  ...
  it('should focus and blur when calling focus() and blur() and rendered inline', async () => {
    const colorPicker = await fixture<AWCColorPicker>(html` <awc-color-picker inline></awc-color-picker> `);
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();
  ...
  describe('when submitting a form', () => {
    it('should serialize its name and value with FormData', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <awc-color-picker name="a" value="#ffcc00"></awc-color-picker>
        </form>
      `);
    ...
    it('should be present in form data when using the form attribute and located outside of a <form>', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <awc-color-picker form="f" name="a" value="#ffcc00"></awc-color-picker>
        </div>
      `);
  ...
  describe('when resetting a form', () => {
    it('should reset the element to its initial value', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <awc-color-picker name="a" value="#ffffff"></awc-color-picker>
          <awc-button type="reset">Reset</awc-button>
        </form>
      `);
      const button = form.querySelector('awc-button')!;
      const colorPicker = form.querySelector('awc-color-picker')!;
  ...
  describe('when using constraint validation', () => {
    ...
    it('should be invalid when required and empty', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker required></awc-color-picker> `);
      expect(el.checkValidity()).to.be.false;
    });

    it('should be invalid when required and disabled is removed', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker disabled required></awc-color-picker> `);
      el.disabled = false;
      await el.updateComplete;
      expect(el.checkValidity()).to.be.false;
    });
    ...
    it('should receive the correct validation attributes ("states") when valid', async () => {
      const el = await fixture<AWCColorPicker>(html` <awc-color-picker required value="#fff"></awc-color-picker> `);
  ...
  runFormControlBaseTests('awc-color-picker');
});
