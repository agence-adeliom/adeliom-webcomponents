...
export const Opacity: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Use the `opacity` attribute to enable the opacity slider. When this is enabled, the value will be displayed as HEXA, RGBA, HSLA, or HSVA based on `format`."
      }
    }
  }
};
...
export const Swatches: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Use the `swatches` attribute to add convenient presets to the color picker. Any format the color picker can parse is acceptable (including CSS color names), but each value must be separated by a semicolon (`;`). Alternatively, you can pass an array of color values to this property using JavaScript."
      },
      story: {
        height: "480px"
      }
