...
const waitForScrollButtonsToBeRendered = async (tabGroup: AWCTabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('awc-icon-button');
    return scrollButtons?.length === 2;
  });
};

const getClientRectangles = (tabGroup: AWCTabGroup): ClientRectangles => {
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = queryByTestId<AWCTab>(container, dataTestId);
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    const tabPanels = Array.from(container.getElementsByTagName('awc-tab-panel'));
    const activeTabPanels = tabPanels.filter((element: AWCTabPanel) => element.hasAttribute('active'));
...
const expectPromiseToHaveName = async (showEventPromise: Promise<AWCTabShowEvent>, expectedName: string) => {
  const showEvent = await showEventPromise;
  expect(showEvent.detail.name).to.equal(expectedName);
};

const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<AWCTab> => {
  const generalHeader = queryByTestId<AWCTab>(container, headerTestId);
  await waitUntil(() => {
...
describe('<awc-tab-group>', () => {
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<AWCTabGroup>(html`
      <awc-tab-group>
        <awc-tab slot="nav" panel="general" data-testid="general-tab-header">General</awc-tab>
        <awc-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</awc-tab>
        <awc-tab-panel name="general">This is the general tab panel.</awc-tab-panel>
  ...
  it('shows the first tab to be active by default', async () => {
    const tabGroup = await fixture<AWCTabGroup>(html`
      <awc-tab-group>
        ...
        <awc-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</awc-tab-panel>
  ...
  describe('scrolling behavior', () => {
    const generateTabs = (n: number): HTMLTemplateResult[] => {
      ...
      for (let i = 0; i < n; i++) {
        result.push(
          html`<awc-tab slot="nav" panel="tab-${i}">Tab ${i}</awc-tab>
            <awc-tab-panel name="tab-${i}">Content of tab ${i}0</awc-tab-panel> `
        );
      }
      return result;
    };
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<AWCTabGroup>(html`<awc-tab-group> ${generateTabs(30)} </awc-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<AWCTabGroup>(html`<awc-tab-group> ${generateTabs(2)} </awc-tab-group>`);
    ...
    it.skip('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<AWCTabGroup>(html`<awc-tab-group> ${generateTabs(50)} </awc-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it.skip('does scroll on scroll button click', async () => {
      ...
      const tabGroup = await fixture<AWCTabGroup>(
        html`<awc-tab-group> ${generateTabs(numberOfElements)} </awc-tab-group>`
      );
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('awc-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    ...
    const expectGeneralTabToBeStillActiveAfter = async (
      ...
      oneEvent(tabGroup, 'awc-tab-show').then(() => (showEventFired = true));
      oneEvent(tabGroup, 'awc-tab-hide').then(() => (hideEventFired = true));
      await action();
    ...
    it('selects a tab by clicking on it', async () => {
      const tabGroup = await fixture<AWCTabGroup>(html`
        <awc-tab-group>
          <awc-tab slot="nav" panel="general" data-testid="general-header">General</awc-tab>
          <awc-tab slot="nav" panel="custom" data-testid="custom-header">Custom</awc-tab>
          <awc-tab-panel name="general">This is the general tab panel.</awc-tab-panel>
          <awc-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</awc-tab-panel>
        </awc-tab-group>
      `);
    ...
    it('does not change if a disabled tab is clicked', async () => {
      const tabGroup = await fixture<AWCTabGroup>(html`
        <awc-tab-group>
          <awc-tab slot="nav" panel="general" data-testid="general-header">General</awc-tab>
          <awc-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</awc-tab>
          <awc-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</awc-tab-panel>
          <awc-tab-panel name="disabled">This is the disabled tab panel.</awc-tab-panel>
        </awc-tab-group>
      `);
    ...
    it('does not allow selection of disabled tabs with arrow keys', async () => {
      const tabGroup = await fixture<AWCTabGroup>(html`
        <awc-tab-group>
          ...
          <awc-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</awc-tab-panel>
          <awc-tab-panel name="disabled">This is the custom tab panel.</awc-tab-panel>
        </awc-tab-group>
      `);
