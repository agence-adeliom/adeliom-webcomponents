...
export default class AWCPagination extends AWCElement {
  ...
  @state() renderedPaginationArray: number[] = [];
    ...
    return html`
      ...
      >
        ...
        <ul part="items" class="pagination__items">
          ${this.renderedPaginationArray.map((pageNumber: number) => {
            return html`
              <li
                ${pageNumber === this.current ? 'aria-current="page"' : ''}
                class=${classMap({
                  pagination__item: true,
                  'pagination__item--current': pageNumber === this.current
                })}
              >
                <awc-button variant="ghost" @click=${() => this.handleClick(pageNumber)}> ${pageNumber} </awc-button>
              </li>
            `;
          })}
        </ul>
