...
import AWCCarouselItem from '../carousel-item/carousel-item.js';
...
export default class AWCCarousel extends AWCElement {
  ...
  private readonly slides = this.getElementsByTagName('awc-carousel-item');
  ...
  private handleScrollEnd() {
    ...
    if (firstIntersecting) {
      this.activeSlide = slides.indexOf(firstIntersecting.target as AWCCarouselItem);
    }
  }
  ...
  private handleSlotChange = (mutations: MutationRecord[]) => {
    const needsInitialization = mutations.some(mutation =>
      [...mutation.addedNodes, ...mutation.removedNodes].some(
        node => AWCCarouselItem.isCarouselItem(node) && !(node as HTMLElement).hasAttribute('data-clone')
      )
    );
