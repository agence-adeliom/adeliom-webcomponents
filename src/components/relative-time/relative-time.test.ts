...
interface AWCRelativeTimeTestCase {
  date: Date;
  expectedOutput: string;
}

const extractTimeElement = (relativeTime: AWCRelativeTime): HTMLTimeElement | null => {
  return relativeTime.shadowRoot?.querySelector('time') || null;
};

const expectFormattedRelativeTimeToBe = async (
  relativeTime: AWCRelativeTime,
  expectedOutput: string
): Promise<void> => {
...
const createRelativeTimeWithDate = async (relativeDate: Date): Promise<AWCRelativeTime> => {
  const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
    <awc-relative-time lang="en-US"></awc-relative-time>
  `);
  relativeTime.date = relativeDate;
  return relativeTime;
};

const expectTitleToMatchLocalizedTimeString = (relativeTime: AWCRelativeTime) => {
...
const testCases: AWCRelativeTimeTestCase[] = [
  {
...
describe('awc-relative-time', () => {
  ...
  describe('handles time correctly', () => {
    ...
    testCases.forEach(testCase => {
      ...
      it(`shows the correct relative time given a String object: ${testCase.expectedOutput}`, async () => {
        ...
        const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
          <awc-relative-time lang="en-US" date="${dateString}"></awc-relative-time>
        `);
    ...
    it('always shows numeric if requested via numeric property', async () => {
      const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
        <awc-relative-time lang="en-US" numeric="always"></awc-relative-time>
      `);
    ...
    it('shows human readable form if appropriate and numeric property is auto', async () => {
      const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
        <awc-relative-time lang="en-US" numeric="auto"></awc-relative-time>
      `);
      relativeTime.date = yesterday;
    ...
    it('allows to use a short form of the unit', async () => {
      ...
      const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
        <awc-relative-time lang="en-US" numeric="always" format="short"></awc-relative-time>
      `);
    ...
    it('allows to use a long form of the unit', async () => {
      const twoYearsAgo = new Date(currentTime.getTime() - 2 * nonLeapYearInSeconds);
      const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
        <awc-relative-time lang="en-US" numeric="always" format="long"></awc-relative-time>
      `);
      relativeTime.date = twoYearsAgo;
    ...
    it('is formatted according to the requested locale', async () => {
      const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
        <awc-relative-time lang="de-DE" numeric="auto"></awc-relative-time>
      `);
      relativeTime.date = yesterday;
  ...
  it('does not display a time element on invalid time string', async () => {
    ...
    const relativeTime: AWCRelativeTime = await fixture<AWCRelativeTime>(html`
      <awc-relative-time lang="en-US" date="${invalidDateString}"></awc-relative-time>
    `);
