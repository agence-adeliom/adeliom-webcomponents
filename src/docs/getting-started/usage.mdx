...
<Meta title="Getting started/Usage" parameters={{ docs: { toc: { disable: false } } }} />
...
As a result, you should almost always listen to custom events instead. For example, instead of listening to `click` to determine when an `<awc-checkbox>` gets toggled, listen to `awc-change`.
...
Some components have methods you can call to trigger various behaviors. For example, you can set focus on an Adeliom WebComponents input using the `focus()` method.
...
Many components use slots to accept content inside them. The most common slot is the _default_ slot, which includes any content inside the component that doesn't have a `slot` attribute.
...
1. [Install Adeliom WebComponents locally](?path=/docs/getting-started-installation--documentation#local-installation)
...
If you are using a [JetBrains IDE](https://www.jetbrains.com/) and you are installing Adeliom WebComponents from NPM, the editor will automatically detect the `web-types.json` file from the package and you should immediately see component information in your editor.

If you are installing from the CDN, you can [download a local copy](https://cdn.jsdelivr.net/npm/@agence-adeliom/awc@%VERSION%/%CDNDIR%/web-types.json) and add it to the root of your project.

### Other Editors

Most popular editors support custom code completion with a bit of configuration. Please [submit a feature request](https://github.com/agence-adeliom/adeliom-webcomponents/issues/new/choose) for your editor of choice. PRs are also welcome!
