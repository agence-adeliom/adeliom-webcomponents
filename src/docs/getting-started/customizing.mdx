...
<Meta title="Getting started/Customizing" parameters={{ docs: { toc: { disable: false } } }} />

# Customizing

Adeliom WebComponents components can be customized at a high level through design tokens. This gives you control over theme colors and general styling. For more advanced customizations, you can make use of CSS parts and custom properties to target individual components.
...
To customize a design token, simply override it in your stylesheet using a `:root` block. Here's an example that changes the primary theme.

```css
:root {
  --awc-color-primary-50: rgb(253, 242, 249);
  --awc-color-primary-200: rgb(250, 208, 234);
  --awc-color-primary-400: rgb(241, 117, 188);
  --awc-color-primary-600: rgb(215, 43, 128);
  --awc-color-primary-800: rgb(154, 26, 84);
  --awc-color-primary-950: rgb(78, 9, 39);
}
```

Many design tokens are described further along in this documentation. For a complete list, refer to `src/themes/light.css` in the project's [source code](https://github.com/agence-adeliom/adeliom-webcomponents/blob/current/src/themes/light.css).
...
<Canvas of={ButtonStories.Tomato} />
