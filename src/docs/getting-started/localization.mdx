...
<Meta title="Getting started/Localization" parameters={{ docs: { toc: { disable: false } } }} />
...
<html lang="es">
  <head>
    <script type="module" src="/path/to/awc/%NPMDIR%/translations/es.js"></script>
  </head>
...
Adeliom WebComponents ships with a number of translations. The default is English (US), which also serves as the fallback locale. As such, you do not need to import the English translation. To see a list of all available translations in the latest version, [refer to this directory](https://github.com/agence-adeliom/adeliom-webcomponents/tree/current/src/translations).
...
- If you're using a bundler, import them from `@agence-adeliom/awc/translations/[lang].js`
...
import('/path/to/awc/%NPMDIR%/translations/de.js');
```
...
To contribute new translations or improvements to existing translations, please submit a pull request on GitHub. Translations are located in [`src/translations`](https://github.com/agence-adeliom/adeliom-webcomponents/blob/main/src/translations) and can be edited directly on GitHub if you don't want to clone the repo locally.
...
If you have any questions, please start a [discussion](https://github.com/agence-adeliom/adeliom-webcomponents/discussions).
...
```js
import { registerTranslation } from '@agence-adeliom/awc';
import type { Translation } from '@agence-adeliom/awc';
