...
You can load Adeliom WebComponents via CDN or by installing it locally. If you're using a framework, make sure to check out the pages for [React](?path=/docs/framework-react--docs), [Vue](?path=/docs/framework-vue--docs), and [Angular](?path=/docs/framework-angular--docs) for additional information.

The CDN loader registers all Adeliom WebComponents elements up front. Note that, if you're only using a handful of components, it will be much more efficient to stick with the autoloader.
...
The code above will load the light theme. If you want to use the [dark theme](?path=/docs/getting-started-themes--docs#dark-theme) instead, update the stylesheet as shown below and add `<html class="awc-theme-dark">` to your page.

```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@agence-adeliom/awc@%VERSION%/%NPMDIR%/themes/dark.css" />
```

### Light & Dark Theme

If you want to load the light or dark theme based on the user's `prefers-color-scheme` setting, use the stylesheets below. The `media` attributes ensure that only the user's preferred theme stylesheet loads and the `onload` attribute sets the appropriate [theme class](?path=/docs/getting-started-themes--docs) on the `<html>` element.
...
Now you can [start using Adeliom WebComponents!](?path=/docs/getting-started-usage--docs)
...
> 📘 Info
>
> An easy way to make sure the base path is configured properly is to check if [icons](/?path=/docs/components-icon--docs) are loading.
...
<script type="module" data-awc="/path/to/awc/%NPMDIR%">
  import '@agence-adeliom/awc/components/button/button.js';
...
```diff
- import AWCButton from '@agence-adeliom/awc/components/button/button.js';
+ import AWCButton from '@agence-adeliom/awc/components/button/button.component.js';
```
...
{
from: path.resolve(**dirname, 'node_modules/@agence-adeliom/awc/%NPMDIR%/assets'),
to: path.resolve(\_\_dirname, 'dist/awc/assets')
}
]
})
]
...
Encore
.setOutputPath(path.resolve(**dirname, 'dist'))
.setPublicPath('/dist')
...
.copyFiles({
from: path.resolve(**dirname, 'node_modules/@agence-adeliom/awc/%NPMDIR%/assets'),
...
import '@agence-adeliom/awc/components/button/button.js';
import '@agence-adeliom/awc/components/icon/icon.js';
import '@agence-adeliom/awc/components/input/input.js';
import '@agence-adeliom/awc/components/rating/rating.js';
import { setBasePath } from '@agence-adeliom/awc';
