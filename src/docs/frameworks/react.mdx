...
```jsx
import { AWCButton } from '@agence-adeliom/awc/react';
...
Many Adeliom WebComponents components emit [custom events](https://developer.mozilla.org/en-US/docs/Web/API/CustomEvent). For example, the [input component](?path=/docs/components-input--docs) emits the `awc-input` event when it receives input. In React, you can listen for the event using `onSlInput`.
...
import { useState } from 'react';
import { AWCInput } from '@agence-adeliom/awc/react';
import type AWCInputElement from '@agence-adeliom/awc/components/input/input';
...
import { AWCInput, AWCInputEvent } from '@agence-adeliom/awc/react';
import type AWCInputElement from '@agence-adeliom/awc/components/input/input';
