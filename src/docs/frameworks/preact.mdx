...
<Meta title="Framework/Preact" parameters={{ docs: { toc: { disable: false } } }} />
...
```jsx
import { AWCInput, AWCQrCode } from "@agence-adeliom/awc/react";
  ...
  return (
    <div>
      ...
      <AWCInput
        ...
        clearable
        label="Value"
        onAwcChange={(event) => setQrCode(event.target.value)}
      ></AWCInput>

      <AWCQrCode value={qrCode}></AWCQrCode>
    </div>
  );
};
