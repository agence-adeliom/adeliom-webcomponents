...
> 📘 **Tip**
>
> Currently, the source of design tokens is considered to be [`light.css`](https://github.com/agence-adeliom/adeliom-webcomponents/blob/main/src/themes/light.css). The dark theme, [dark.css](https://github.com/agence-adeliom/adeliom-webcomponents/blob/main/src/themes/dark.css), mirrors all of the same tokens with dark mode-specific values where appropriate. Work is planned to move all design tokens to a single file, perhaps JSON or YAML, in the near future.
...
| Token                           | Value                         |
| ------------------------------- | ----------------------------- |
| `--awc-button-font-size-small`  | `var(--awc-font-size-xsmall)` |
| `--awc-button-font-size-medium` | `var(--awc-font-size-small)`  |
| `--awc-button-font-size-large`  | `var(--awc-font-size-medium)` |

## Form Inputs

Form input tokens control the appearance of form controls such as [input](?path=/docs/components-input--documentation), [select](?path=/docs/components-select--documentation), [textarea](?path=/docs/components-textarea--documentation), etc.
...
| Token                                    | Value                          |
| ---------------------------------------- | ------------------------------ |
| `--awc-input-help-text-font-size-small`  | `var(--awc-font-size-xsmall)`  |
...
Toggle tokens control the appearance of toggles such as [checkbox](?path=/docs/components-checkbox--documentation), [radio](?path=/docs/components-radio--documentation), [switch](?path=/docs/components-switch--documentation), etc.
...
Overlay tokens control the appearance of overlays as used in [dialog](?path=/docs/components-dialog--documentation), [drawer](?path=/docs/components-drawer--documentation), etc.
...
Panel tokens control the appearance of panels such as those used in [dialog](?path=/docs/components-dialog--documentation), [drawer](?path=/docs/components-drawer--documentation), [menu](?path=/docs/components-menu--documentation), etc.
...
Tooltip tokens control the appearance of tooltips. This includes the [tooltip](?path=/docs/components-tooltip--documentation) component as well as other implementations, such [range tooltips](?path=/docs/components-range--documentation).
...
| `--awc-tooltip-line-height`      | `var(--awc-line-height-normal)`                        |
| `--awc-tooltip-padding`          | `var(--awc-spacing-2xsmall) var(--awc-spacing-xsmall)` |
