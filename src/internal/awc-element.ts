...
const awcEventToEvent: Record<string, string> = {
  'awc-blur': 'blur',
  'awc-change': 'change',
  'awc-clear': 'clear',
  'awc-focus': 'focus',
...
export default class AWCElement extends LitElement {
  ...
  emit<T extends string & keyof ValidEventTypeMap>(
    ...
    if (awcEventToEvent[name]) {
      this.dispatchEvent(new Event(awcEventToEvent[name], { bubbles: true, cancelable: false, composed: true }));
    }
  ...
  static dependencies: Record<string, typeof AWCElement> = {};

  constructor() {
    super();
    Object.entries((this.constructor as typeof AWCElement).dependencies).forEach(([name, component]) => {
      (this.constructor as typeof AWCElement).define(name, component);
    });
  }
}
