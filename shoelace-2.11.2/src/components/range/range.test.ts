...
describe('<awc-range>', () => {
  it('should pass accessibility tests', async () => {
    const el = await fixture<SlRange>(html` <awc-range label="Name"></awc-range> `);
    await expect(el).to.be.accessible();
  });

  it('default properties', async () => {
    const el = await fixture<SlRange>(html` <awc-range></awc-range> `);
  ...
  it('should have title if title attribute is set', async () => {
    const el = await fixture<SlRange>(html` <awc-range title="Test"></awc-range> `);
  ...
  it('should be disabled with the disabled attribute', async () => {
    const el = await fixture<SlRange>(html` <awc-range disabled></awc-range> `);
  ...
  describe('when the value changes', () => {
    it('should emit awc-change and awc-input when the value changes from clicking the slider', async () => {
      const el = await fixture<SlRange>(html` <awc-range value="0"></awc-range> `);
    ...
    it('should emit awc-change and awc-input and decrease the value when pressing left arrow', async () => {
      const el = await fixture<SlRange>(html` <awc-range value="50"></awc-range> `);
      const changeHandler = sinon.spy();
      const inputHandler = sinon.spy();
    ...
    it('should not emit awc-change or awc-input when stepUp() is called programmatically', async () => {
      const el = await fixture<SlRange>(html` <awc-range step="2" value="2"></awc-range> `);
  ...
  describe('when submitting a form', () => {
    ...
    it('should receive validation attributes ("states") even when novalidate is used on the parent form', async () => {
      ...
      const range = el.querySelector<SlRange>('awc-range')!;
