...
describe('<awc-tab-group>', () => {
  ...
  describe('scrolling behavior', () => {
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`<awc-tab-group> ${generateTabs(30)} </awc-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`<awc-tab-group> ${generateTabs(2)} </awc-tab-group>`);
    ...
    it.skip('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<SlTabGroup>(html`<awc-tab-group> ${generateTabs(50)} </awc-tab-group>`);
      tabGroup.placement = 'start';
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: AWCTabGroup, action: () => Promise<void>): Promise<void> => {
      ...
      const showEventPromise = oneEvent(tabGroup, 'awc-tab-show') as Promise<SlTabShowEvent>;
      await action();
