...
describe('<awc-qr-code>', () => {
  it('should render a component', async () => {
    const qrCode = await fixture<SlQrCode>(html` <awc-qr-code value="test data"></awc-qr-code>`);
  ...
  it('uses the label if given', async () => {
    const qrCode = await fixture<SlQrCode>(html` <awc-qr-code value="test data" label="test label"></awc-qr-code>`);

    expectCanvasToHaveAriaLabel(qrCode, 'test label');
  });

  it('sets the correct color for the qr code', async () => {
    const qrCode = await fixture<SlQrCode>(html` <awc-qr-code value="test data" fill="red"></awc-qr-code>`);
  ...
  it('has the expected size', async () => {
    const qrCode = await fixture<SlQrCode>(html` <awc-qr-code value="test data" size="100"></awc-qr-code>`);
