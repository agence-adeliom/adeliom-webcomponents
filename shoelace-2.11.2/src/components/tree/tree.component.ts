 ...
 */
export default class AWCTree extends ShoelaceElement {
  ...
  //
  private lastFocusedItem: AWCTreeItem | null;
  ...
  private handleTreeChanged = (mutations: MutationRecord[]) => {
    for (const mutation of mutations) {
      const addedNodes: AWCTreeItem[] = [...mutation.addedNodes].filter(SlTreeItem.isTreeItem) as AWCTreeItem[];
      const removedNodes = [...mutation.removedNodes].filter(SlTreeItem.isTreeItem) as AWCTreeItem[];
  ...
  private getAllTreeItems() {
    return [...this.querySelectorAll<SlTreeItem>('awc-tree-item')];
  }
