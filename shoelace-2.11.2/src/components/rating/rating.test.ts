...
describe('<awc-rating>', () => {
  it('should pass accessibility tests', async () => {
    const el = await fixture<SlRating>(html` <awc-rating label="Test"></awc-rating> `);
  ...
  it('should be readonly with the readonly attribute', async () => {
    const el = await fixture<SlRating>(html` <awc-rating label="Test" readonly></awc-rating> `);
    const base = el.shadowRoot!.querySelector<HTMLElement>('[part~="base"]')!;
  ...
  it('should be disabled with the disabled attribute', async () => {
    const el = await fixture<SlRating>(html` <awc-rating label="Test" disabled></awc-rating> `);
    const base = el.shadowRoot!.querySelector<HTMLElement>('[part~="base"]')!;
  ...
  it('should set max value by attribute', async () => {
    const el = await fixture<SlRating>(html` <awc-rating label="Test" max="12"></awc-rating> `);
    const base = el.shadowRoot!.querySelector<HTMLElement>('[part~="base"]')!;
  ...
  it('should set selected value by attribute', async () => {
    const el = await fixture<SlRating>(html` <awc-rating label="Test" value="3"></awc-rating> `);
    const base = el.shadowRoot!.querySelector<HTMLElement>('[part~="base"]')!;
  ...
  it('should emit awc-change when clicked', async () => {
    const el = await fixture<SlRating>(html` <awc-rating></awc-rating> `);
  ...
  it('should not emit awc-change when disabled', async () => {
    const el = await fixture<SlRating>(html` <awc-rating value="5" disabled></awc-rating> `);
    const lastSymbol = el.shadowRoot!.querySelector<HTMLSpanElement>('.rating__symbol:last-child')!;
    const changeHandler = sinon.spy();
  ...
  it('should not emit awc-change when the value is changed programmatically', async () => {
    const el = await fixture<SlRating>(html` <awc-rating label="Test" value="1"></awc-rating> `);
